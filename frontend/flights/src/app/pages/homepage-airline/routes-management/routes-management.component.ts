...
ngOnInit(): void {
  ...
  // Ora puoi usare this.airlineName per le chiamate API
  this.loadRoutes();
}
...
onCreateRoute() {
  ...
  this.http.post(`${environment.apiUrl}/api/navigate/routes`, {
    origin: from.id,
    destination: to.id
  }, {
    ...
    error: err => {
      this.createError = err.error?.error || 'Errore nella creazione della rotta';
      this.loadingCreate = false;
    }
  });
}
...
private getAirlineNameFromToken(): string | null {
  ...
  this.http.get<{ routes: any[] }>(
    ...
    error: err => {
      this.error = 'Errore nel caricamento delle rotte (possibile assenza di rotte in cui operi)';
      this.loading = false;
    }
  });
}
 ...
 onAirportInput(field: 'from' | 'to') {
  ...
  this.http.post(`${environment.apiUrl}/api/airlines/routes`, {
      departure: from.id,
      destination: to.id
    }, {
      headers: { 'Authorization': `Bearer ${localStorage.getItem('postmessages_token')}` }
      ...
      error: err => {
        this.addError = err.error?.error || 'Errore nell\'aggiunta della rotta';
        this.loadingAdd = false;
      }
    });
}
