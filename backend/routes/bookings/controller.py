...
# Funzione globale per generazione PDF in processo separato
def pdf_job(ticket_data):
    import asyncio
    from routes.bookings.ticket.ticketGenerator import generate_ticket_pdf as generateTicketPDF
    return asyncio.run(generateTicketPDF(ticket_data))
...
def downloadBooking(id):
    ...
    # Genera PDF in un processo separato per evitare problemi di signal/thread
    with concurrent.futures.ProcessPoolExecutor() as executor:
        future = executor.submit(pdf_job, ticket_data)
